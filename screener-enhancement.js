window.launchScreenerEnhancement = function () {
  ...
  const selectedQuarterlyRows = getSelectedRows(
    quartyerlyTable,
    ROWS_TO_SELECT
  ).reverse();
  const areResultsHalfYearly = quartyerlyTable.textContent?.includes(
    "Half Yearly Results"
  );

  window.growthMode = "yoy";
  addYoYGrowthPopovers(
    selectedQuarterlyRows,
    areResultsHalfYearly,
    ROWS_FOR_GROWTH_TOOLTIP
  );
  ...
  if (document.querySelector("#shareholding")) {
    const totalPromoter =
      getLatestValueFromTable("#shareholding", "Promoter") ?? 0;
    const pledgedInfoDiv = createSectionInfoLabel();

    const pledgedValue = totalPromoter
      ? ((pledgedPercentage / 100) * totalPromoter).toFixed(2)
      : 0;
    if (!pledgeNotFound) {
    ...
    document
      ...
      ?.parentElement?.insertBefore(
        pledgedInfoDiv,
        document.querySelector("#quarterly-shp")
      );
  }

  const {
    value: contingentLiabilities,
    notFound: contingentLiabilitiesNotFound,
  } = getQuickRatio("Cont Liab", true);
  ...
  const toggleBtn = getContainerButton(
    window.growthMode === "qoq" ? "Viewing QoQ" : "Viewing YoY"
  );
  ...
  const latestDepreciation = parseFloat(
    getLatestValueFromTable("#profit-loss", "Depreciation")
  );
  const latestInterestCost = parseFloat(
    getLatestValueFromTable("#profit-loss", "Interest")
  );
  const otherCosts = latestDepreciation + latestInterestCost;

  const shareCapital = getLatestValueFromTable(
    "#balance-sheet",
    "Equity Capital"
  );
  ...
  const { value: enterpriseValue, notFound: evNotFound } =
...
function addAndUpdateMarginRows(tableId) {
  ...
  if (!salesRow || !netProfitRow || !operatingProfitRow) {
    return showAlert(
      "Sales/Revenue, Net Profit, or Operating Profit row not found."
    );
  }
    ...
    Array.from(opmRow.cells)
      ...
      .forEach((cell, index) => {
        ...
        cell.textContent = sales
          ? ((operatingProfit / sales) * 100).toFixed(2) + "%"
          : "-";
      });
  }
}
...
function createSectionInfoLabel() {
  return Object.assign(document.createElement("div"), {
    textContent: "",
    style:
      "text-align: right; font-weight: 500; font-size: 15px; color: #c62828;",
  });
}

function getLatestValueFromTable(tableId, rowLabel, asIs = false) {
  const value = Array.from(
    document.querySelectorAll(`${tableId} table.data-table tbody tr`)
  )
...
function transferRowsToCashFlow(
  selectedRows,
  headerCells,
  cashFlowHeaders,
  cashFlowBody
...
function addSumButton(headerCells, tableBody, tableContainer) {
  ...
  button.addEventListener("click", () =>
    sumSelectedColumns(headerCells, tableBody)
  );
...
const expandCellAndWaitForContent = async (
  ...
  const childCell = getTableCellWithText(
    tableContainer,
    cellSelector,
    childCellText
  );
...
async function sumSelectedColumns(headerCells, tableBody) {
    ...
    if (keywords.some((k) => label.toLowerCase().startsWith(k.toLowerCase()))) {
      const total = selectedIndexes.reduce((sum, index) => {
        const val = parseFloat(
          row.cells[index]?.textContent.replace(/[^0-9.-]+/g, "") || "0"
        );
  ...
  rowSums.sort((a, b) => {
    const aIndex = keywords.findIndex((k) =>
      a.label.toLowerCase().startsWith(k.toLowerCase())
    );
    const bIndex = keywords.findIndex((k) =>
      b.label.toLowerCase().startsWith(k.toLowerCase())
    );
  ...
  const cfo = rowSums.find((rs) =>
    rs.label.startsWith("Cash from Operating Activity")
  );
  const fixedAssetsPurchased = rowSums.find((rs) =>
    rs.label.startsWith("Fixed assets purchased")
  );
  const fixedAssetsSold = rowSums.find((rs) =>
    rs.label.startsWith("Fixed assets sold")
  );
  ...
  if (cfo && pat)
    rowSums.push({
      label: "CFO/PAT",
      total: ((cfo.total / pat.total) * 100)?.toFixed(2) + " %",
    });
  if (cfo && ebitda)
    rowSums.push({
      label: "CFO/EBITDA",
  ...
  const headerDisplay = `<div style="margin-bottom: 32px">${selectedNames.join(
    ", "
  )}</div>`;
...
async function addMoreRatiosToTable() {
  ...
  const sales = salesRows?.length
    ? salesRows
    : getRowValues("Revenue", pnlSection);
  ...
  const debtToEquityMap = createDebtToEquityMap(
    bsHeaders,
    borrowings,
    equityCapital,
    reserves
  ...
  const receivablesGrowthMap = createGrowthMap(
    bsHeaders,
    receivablesPerYearMap
  );
  ...
  const interestCoverageMap = createInterestCoverageMap(
    pnlHeaders,
    pbt,
    interest
  );
  const cfoPatGrowthMap = createCfoRatiosMap(cashflowHeaders, cfo, netProfit);
  const cfoEbitdaGrowthMap = createCfoRatiosMap(
    cashflowHeaders,
    cfo,
    operatingProfit
  );
  ...
  function createDebtToEquityMap(headers, borrowings, equityCapital, reserves) {
    ...
    headers.slice(1).forEach((year, i) => {
      ...
      debtToEquityMap[year] = {
        ratioValue:
          equity && borrow != null ? (borrow / equity).toFixed(2) : null,
        numerator: borrow,
        denominator: equity,
      };
    });
  ...
  function createCfoRatiosMap(headers, cfoValues, earnings) {
    ...
    headers.slice(1).forEach((year, i) => {
      ...
      const cfoToPat =
        profit !== 0 ? ((cfo / profit) * 100).toFixed(2) + "%" : "-";
  ...
  function createReceivablesVsSalesGrowthMap(
    headers,
    receivablesGrowthMap,
    salesGrowthMap
  ) {
    ...
    headers.slice(1).forEach((year) => {
      ...
      map[year] = `${receivablesGrowth.toFixed(0)}% / ${salesGrowth.toFixed(
        0
      )}%`;
    });
    return map;
      ...
      if (ratioValue !== null) {
        ...
        tooltipText =
          numerator != null && denominator != null
            ? `${toolTipLabel}: ${numerator?.toFixed(2)}/${denominator?.toFixed(
                2
              )}`
            : "";
      }
  ...
  function createROCEMap(
    ...
    headers.slice(1).forEach((year, i) => {
      ...
      const equityPrev =
        (equityCapital[i - 1] ?? null) + (reserves[i - 1] ?? null);
      const debtPrev = borrowings[i - 1] ?? null;

      const prevDataExists =
        equityCapital[i - 1] != null ||
        reserves[i - 1] != null ||
        borrowings[i - 1] != null;
      ...
      const roce =
        avgCapitalEmployed !== 0 ? (numerator / avgCapitalEmployed) * 100 : 0;
  ...
  function createROIICMap(
    ...
    headers.slice(1).forEach((year, i) => {
      if (
        i === 0 ||
        !isFinite(investedCapitals[i]) ||
        !isFinite(investedCapitals[i - 1])
      ) {
        roiicMap[year] = {
          ratioValue: "-",
          numerator: null,
...
function createDrawerWithLayoutToggle() {
  ...
  const styles = `
          ...
          #drawer-handle {
            ...
            box-shadow:   ${
              darkMode
                ? "3px 0 4px rgba(250,250,250,0.4)"
  ...
  handle.addEventListener("click", () => {
    ...
    if (isOpening) {
      loadDrawerScript(
        "https://cdn.jsdelivr.net/gh/gitdev-common/utils@1.0.11/screener-guide.js"
      )
        .then(() => {
...
function showFinancialModelModal(
  ...
  function calculateGrowthMultiple(cagrPercent, years) {
    ...
    // Absolute return = (growthMultiple - 1) * 100
    return (growthMultiple - 1) * 100;
  }
  ...
  setTimeout(() => {
    ...
    document.getElementById("calculate-btn")?.addEventListener("click", () => {
      ...
      const entValue = parseFloat(
        document.getElementById("enterpriseValue").value || 0
      );
      ...
      for (const s of ["Bull", "Base", "Bear"]) {
        ...
        if (method === "pe") {
          const revGrowth =
            parseFloat(document.getElementById(`${s}-rev`).value) / 100;
          const ebitdaMargin =
            parseFloat(document.getElementById(`${s}-ebitda`).value) / 100;
          const otherCostVal = parseFloat(
            document.getElementById(`${s}-other`).value
          );
          const taxPct =
            parseFloat(document.getElementById(`${s}-tax`).value) / 100;
          ...
          results.push({
            ...
            cagr: `${cagr.toFixed(2)} % (${calculateGrowthMultiple(
              cagr,
              years
            ).toFixed(1)}%)`,
            forwardMultiple,
          ...
          const multiple = parseFloat(
            document.getElementById(`${s}-evEbitda`).value
          );
          ...
          const netWorthGrowthRate =
            parseFloat(document.getElementById(`${s}-networthGrowth`).value) /
            100;
                ...
                ${results
                  .map((r) => {
                    ...
                    <tr>
                      ...
                      <td>${r.futureMCap}</td>
                      <td style="color:${
                        r.cagr.includes("-") ? "#c62828" : "#2e7d32"
                      }; font-weight:600;">${r.cagr}</td>
                    </tr>`;
                  })
                  .join("")}
              </tbody>
